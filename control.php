...
define('PHP_SOCKET', (exec('grep "^listen =" /data/data/com.termux/files/usr/etc/php-fpm.d/www.conf 2>/dev/null | awk "{print \$3}"') ?: 'unix:/data/data/com.termux/files/usr/var/run/php-fpm.sock'));
...
function getSitesConfig() {
    ...
    return json_decode(file_get_contents(SITES_JSON), true) ?: [];
}

function saveSitesConfig($config) {
    $dir = dirname(SITES_JSON);
    if (!is_dir($dir)) mkdir($dir, 0755, true);
    file_put_contents(SITES_JSON, json_encode($config, JSON_PRETTY_PRINT));
}

function getNextPort() {
    $used = [8080];
    $config = getSitesConfig();
    foreach ($config as $s) { if (!empty($s['port'])) $used[] = $s['port']; }
    for ($p = 8081; $p <= 8999; $p++) { if (!in_array($p, $used)) return $p; }
    return 0;
}

function normalizeSitePath($name) {
    $path = SITES_DIR . '/' . $name . '/public_html';
...
function generateNginxBlock($name, $domain, $port, $path) {
    $real = realpath($path);
    if (!$real) return '';
    return "server {\n"
        . "    listen $port;\n"
...
function getPhpSocket() {
    $socketPath = '/data/data/com.termux/files/usr/var/run/php-fpm.sock';
    $confFile = '/data/data/com.termux/files/usr/etc/php-fpm.d/www.conf';
    if (is_file($confFile)) {
        $s = trim(exec("grep '^listen =' $confFile 2>/dev/null | awk '{print \$3}'"));
        if ($s) return $s;
    }
        ...
        if ($action === 'restart_all') {
            ...
            if (!preg_match('/^[a-z0-9_-]+$/', $name)) {
                ...
                if (is_dir($publicHtml)) {
                    ...
                    $block = generateNginxBlock($name, $domain, $port, $publicHtml);
            ...
            if ($name && preg_match('/^[a-z0-9_-]+$/', $name)) {
                ...
                if ($target) {
                    exec("rm -rf " . escapeshellarg(dirname($target) === $publicHtml ? dirname($target) : $target), $raw, $rc);
                    $config = getSitesConfig();
                    unset($config[$name]);
                    saveSitesConfig($config);
            ...
            if (isset($config[$name])) {
                ...
                if ($config[$name]['enabled']) {
                    file_put_contents($target, generateNginxBlock($name, $config[$name]['domain'], $config[$name]['port'], $config[$name]['path']));
                } else {
                    @unlink($target);
                }
                restartNginx();
            ...
            if (!$isRoot) {
                ...
                foreach ($config as $name => $site) {
                    if (!empty($site['domain']) && $site['enabled']) {
...
?>
<?php if (!$logged_in): ?>
<?php include __DIR__ . '/panel/login.php'; ?>
<?php exit; endif; ?>
